...
@objc protocol SideMenuDelegate {
    @objc optional func sideMenu(sideMenu: SideMenu, didRecognizePanGesture recognizer: UIPanGestureRecognizer)
    @objc optional func sideMenu(sideMenu: SideMenu, willShowMenuViewController menuViewController: UIViewController)
    @objc optional func sideMenu(sideMenu: SideMenu, didShowMenuViewController menuViewController: UIViewController)
    @objc optional func sideMenu(sideMenu: SideMenu, willHideMenuViewController menuViewController: UIViewController)
    @objc optional func sideMenu(sideMenu: SideMenu, didHideMenuViewController menuViewController: UIViewController)
}

@IBDesignable public class SideMenu : UIViewController, UIGestureRecognizerDelegate {
    ...
    override public func viewDidLoad() {
        ...
        button.addTarget(self, action: #selector(SideMenu.hideMenuViewController), for: UIControl.Event.touchUpInside)
        ...
        self.menuViewContainer.autoresizingMask = [UIView.AutoresizingMask.flexibleWidth, UIView.AutoresizingMask.flexibleHeight]
        
        if (self.leftMenuViewController != nil) {
            self.addChild(self.leftMenuViewController!)
            self.leftMenuViewController!.view.frame = self.view.bounds
            self.leftMenuViewController!.view.autoresizingMask = [UIView.AutoresizingMask.flexibleWidth, UIView.AutoresizingMask.flexibleHeight]
            self.menuViewContainer.addSubview(self.leftMenuViewController!.view)
            self.leftMenuViewController?.didMove(toParent: self)
        }
        
        if (self.rightMenuViewController != nil) {
            self.addChild(self.rightMenuViewController!)
            self.rightMenuViewController?.view.frame = self.view.bounds
            self.rightMenuViewController?.view.autoresizingMask = [UIView.AutoresizingMask.flexibleWidth, UIView.AutoresizingMask.flexibleHeight]
            self.menuViewContainer.addSubview(self.rightMenuViewController!.view)
            self.rightMenuViewController?.didMove(toParent: self)
        }
        
        self.contentViewContainer.frame = self.view.bounds
        self.contentViewContainer.autoresizingMask = [UIView.AutoresizingMask.flexibleWidth, UIView.AutoresizingMask.flexibleHeight]
        ...
        if (self.panGestureEnabled) {
            ...
            let panGestureRecognizer: UIPanGestureRecognizer = UIPanGestureRecognizer(target: self, action: #selector(SideMenu.panGestureRecognized(recognizer:)))
    ...
    public func presentLeftMenuViewController() {
        self.presentMenuViewContainerWithMenuViewController(menuViewController: self.leftMenuViewController!)
        self.showLeftMenuViewController()
    }
    
    public func presentRightMenuViewController() {
        self.presentMenuViewContainerWithMenuViewController(menuViewController: self.rightMenuViewController!)
        self.showRightMenuViewController()
    }
    
    @objc public func hideMenuViewController() {
        self.hideMenuViewControllerAnimated(animated: true)
    }
        ...
        if (!animated) {
            ...
            UIView.animate(withDuration: self.animationDuration, animations: {
                contentViewController.view.alpha = 1
                    ...
                    self.hideViewController(viewController: self.contentViewController!)
    ...
    func showLeftMenuViewController() {
        ...
        UIView.animate(withDuration: self.animationDuration, animations: {
            ...
            self.contentViewContainer.center = CGPoint(x: (UIDevice.current.orientation.isLandscape ? self.contentViewInLandscapeOffsetCenterX + self.view.frame.width : self.contentViewInPortraitOffsetCenterX + self.view.frame.width), y: self.contentViewContainer.center.y)
    ...
    func showRightMenuViewController() {
        ...
        UIView.animate(withDuration: self.animationDuration, animations: {
            ...
            self.contentViewContainer.center = CGPoint(x: (UIDevice.current.orientation.isLandscape ? -self.contentViewInLandscapeOffsetCenterX : -self.contentViewInPortraitOffsetCenterX), y: self.contentViewContainer.center.y)
    ...
    func hideMenuViewControllerAnimated(animated: Bool) {
        ...
        let completionBlock = { [weak self] in
            ...
            if (self!.visible == false) {
                self!.delegate?.sideMenu?(sideMenu: self!, didHideMenuViewController:rightMenuVisible ? self!.rightMenuViewController! : self!.leftMenuViewController!)
            }
        }
    ...
    func addContentButton() {
        ...
        self.contentButton.autoresizingMask = [UIView.AutoresizingMask.flexibleWidth , UIView.AutoresizingMask.flexibleHeight]
    ...
    func addMenuViewControllerMotionEffects() {
        if (self.parallaxEnabled) {
            ...
            let interpolationHorizontal: UIInterpolatingMotionEffect = UIInterpolatingMotionEffect.init(keyPath: "center.x", type: UIInterpolatingMotionEffect.EffectType.tiltAlongHorizontalAxis)
            ...
            let interpolationVertical: UIInterpolatingMotionEffect = UIInterpolatingMotionEffect.init(keyPath: "center.y", type: UIInterpolatingMotionEffect.EffectType.tiltAlongVerticalAxis)
    ...
    public func gestureRecognizer(_ gestureRecognizer: UIGestureRecognizer, shouldReceive touch: UITouch) -> Bool {
        if (self.interactivePopGestureRecognizerEnabled && self.contentViewController!.isKind(of: UINavigationController.classForCoder())) {
        ...
        if (self.panFromEdge && gestureRecognizer.isKind(of: UIPanGestureRecognizer.classForCoder()) && !self.visible) {
    ...
    @objc func panGestureRecognized(recognizer: UIPanGestureRecognizer) {
        ...
        if (recognizer.state == UIGestureRecognizer.State.changed) {
            ...
            if (!self.didNotifyDelegate) {
                if (point.x > 0) {
                    if (!self.visible) {
                        self.delegate?.sideMenu?(sideMenu: self, willShowMenuViewController:self.leftMenuViewController!)
                    }
                } // Dragging right menu Controller in disabled
//                if (point.x < 0) {
//                    if (!self.visible) {
...
//                }
        ...
        if (recognizer.state == UIGestureRecognizer.State.ended) {
            ...
            if (self.panMinimumOpenThreshold > 0 && ((self.contentViewContainer.frame.origin.x < 0 && self.contentViewContainer.frame.origin.x > -(CGFloat(self.panMinimumOpenThreshold))) ||
                ...
                self.hideMenuViewControllerAnimated(animated: false)
            }
            else {
    ...
    public var leftMenuViewController: UIViewController? {
        ...
        set {
            ...
            self.hideViewController(viewController: self._leftMenuViewController!)
            self._leftMenuViewController = newValue
            
            self.addChild(self._leftMenuViewController!)
            self._leftMenuViewController!.view.frame = self.view.bounds
            self._leftMenuViewController!.view.autoresizingMask = [UIView.AutoresizingMask.flexibleWidth , UIView.AutoresizingMask.flexibleHeight]
            self.menuViewContainer.addSubview(self._leftMenuViewController!.view)
            self._leftMenuViewController!.didMove(toParent: self)
    ...
    public var rightMenuViewController: UIViewController? {
        ...
        set {
            ...
            self.hideViewController(viewController: self._rightMenuViewController!)
            self._rightMenuViewController = newValue
            
            self.addChild(self._rightMenuViewController!)
            self._rightMenuViewController!.view.frame = self.view.bounds
            self._rightMenuViewController!.view.autoresizingMask = [UIView.AutoresizingMask.flexibleWidth , UIView.AutoresizingMask.flexibleHeight]
            self.menuViewContainer.addSubview(self._rightMenuViewController!.view)
            self._rightMenuViewController!.didMove(toParent: self)
    ...
    public override func willAnimateRotation(to toInterfaceOrientation: UIInterfaceOrientation, duration: TimeInterval) {
        if (self.visible) {
            ...
            if (self.leftMenuVisible) {
                center = CGPoint(x: (UIDevice.current.orientation.isLandscape ? self.contentViewInLandscapeOffsetCenterX + self.view.frame.width : self.contentViewInPortraitOffsetCenterX + self.view.frame.width),
                                 y: self.contentViewContainer.center.y)
            } else {
                center = CGPoint(x: (UIDevice.current.orientation.isLandscape ? -self.contentViewInLandscapeOffsetCenterX : -self.contentViewInPortraitOffsetCenterX),
                                 y: self.contentViewContainer.center.y)
            }
